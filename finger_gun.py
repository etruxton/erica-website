...
@finger_gun_bp.route('/process_frame', methods=['POST'])
def process_frame():
    try:
        ...
        processed_frame = process_image(frame, session) #added session
...
def process_image(frame, session):
...
def generate_frames():
    while True:
        ...
        time.sleep(0.050) # 20 fps delay.

@finger_gun_bp.route('/processed_stream')
